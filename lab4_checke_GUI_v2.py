		...
		try:
			for folder, _, files in w:
				...
				with open(fin) as ffin, open(fmy, 'w') as ffmy, open(ferr, 'w') as fferr:
					if subprocess.run(args, cwd=frisc_folder_var.get(), stdin=ffin, stderr=fferr, shell=True).returncode != 0:
						...
						pass;
					pass;
					if subprocess.run([path_to_node, path_to_main_js, ffrisc], stdout=ffmy, stderr=fferr, shell=True).returncode != 1: # main.js uvijek vraća 1?
